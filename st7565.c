#include "st7565-config.h"
#include "st7565.h"

/** Global buffer to hold the current screen contents. */
// This has to be kept here because the width & height are set in
// st7565-config.h
unsigned char glcd_buffer[SCREEN_WIDTH * SCREEN_HEIGHT / 8];

#ifdef ST7565_DIRTY_PAGES
unsigned int8 glcd_dirty_pages;
#endif

/* Alle möglichen SPI Modi */
#define SPI_MODE_0 (SPI_L_TO_H | SPI_XMIT_L_TO_H)
#define SPI_MODE_1 (SPI_L_TO_H)
#define SPI_MODE_2 (SPI_H_TO_L)
#define SPI_MODE_3 (SPI_H_TO_L | SPI_XMIT_L_TO_H)

void init_spi() {
    setup_spi(SPI_MASTER | SPI_MODE_0 | SPI_CLK_DIV_16);
    //SETUP_ADC_PORTS(NO_ANALOGS);
}

//void glcd_send(int8 spi_data, int8 a0) {
//    // A0 = ?H?: Indicates that D0 to D7 are display data
//    // A0 = ?L?: Indicates that D0 to D7 are control data.
//    //set / clear A0-Bit
//    if (a0)
//        output_high(GLCD_A0); // display data
//    else
//        output_low(GLCD_A0); // control data
//    // Byte senden
//    spi_write(spi_data);
//
//}
//#define glcd_send_command(command)      glcd_send(command, 0)
//#define glcd_send_display_data(data)    glcd_send(data, 1)

void glcd_hw_reset() {
    output_high(GLCD_RESET); // RES\ = HIGH (kein Reset)
    delay_ms(100);
    output_low(GLCD_RESET); // RES\ = LOW (Reset)
}

void glcd_pixel(char x, char y, char colour) {

    if (x > SCREEN_WIDTH || y > SCREEN_HEIGHT) return;

    // Real screen coordinates are 0-63, not 1-64.
    if (x > 0) x -= 1;
    if (y > 0) y -= 1;

    unsigned int16 array_pos = (int16)x + (((int16)y / 8) * 128);

#ifdef ST7565_DIRTY_PAGES
#warning ** ST7565_DIRTY_PAGES enabled, only changed pages will be written to the GLCD **
    glcd_dirty_pages |= 1 << (array_pos / 128);
#endif

    if (colour) {
        glcd_buffer[array_pos] |= 1 << (y % 8);
    } else {
        glcd_buffer[array_pos] &= 0xFF ^ 1 << (y % 8);
    }
}

void glcd_blank() {
    // Reset the internal buffer
    //    for (int16 n = 1; n <= (SCREEN_WIDTH * SCREEN_HEIGHT / 8) - 1; n++) {
    //        glcd_buffer[n] = 0;
    //    }
    memset(glcd_buffer, 0, sizeof (glcd_buffer[0]) * SCREEN_WIDTH * SCREEN_HEIGHT / 8);

    // Clear the actual screen
    for (int8 y = 0; y < 8; y++) {
        glcd_command(GLCD_CMD_SET_PAGE | y);

        // Reset column to 0 (the left side)
        glcd_command(GLCD_CMD_COLUMN_LOWER);
        glcd_command(GLCD_CMD_COLUMN_UPPER);

        // We iterate to 132 as the internal buffer is 65*132, not
        // 64*124.
        for (unsigned int16 x = 0; x < 132; x++) {
            glcd_data(0x00);
        }
    }
}

void glcd_refresh() {
    for (int y = 0; y < 8; y++) {

#ifdef ST7565_DIRTY_PAGES
        // Only copy this page if it is marked as "dirty"
        if (!(glcd_dirty_pages & (1 << y))) continue;
#endif

        glcd_command(GLCD_CMD_SET_PAGE | y);

        // Reset column to the left side. The internal memory of the
        // screen is 132*64, we need to account for this if the display
        // is flipped.
        //
        // Some screens seem to map the internal memory to the screen
        // pixels differently, the ST7565_REVERSE define allows this to
        // be controlled if necessary.
#ifdef ST7565_REVERSE
        if (!glcd_flipped) {
#else
        if (glcd_flipped) {
#endif
            glcd_command(GLCD_CMD_COLUMN_LOWER | 4);
        } else {
            glcd_command(GLCD_CMD_COLUMN_LOWER);
        }
        glcd_command(GLCD_CMD_COLUMN_UPPER);

        for (int x = 0; x < 128; x++) {
            glcd_data(glcd_buffer[((int16)y * 128) + x]);
        }
    }

#ifdef ST7565_DIRTY_PAGES
    // All pages have now been updated, reset the indicator.
    glcd_dirty_pages = 0;
#endif
}


// EA Dogm 128-6 init

void glcd_init() {
    // Init SPI
    // TODO: Auswertung der defines welcher SPI Port usw.
    init_spi();
    // reset
    glcd_hw_reset();

    // Begin LCD init
    output_low(GLCD_A0); // Following are commands

    glcd_command(0x40); // Display start line 0
#ifdef GLCD_TOP_VIEW
    glcd_command(0xA0); //
    glcd_command(0xC8); //
#else
    glcd_command(0xA1); // ADC reverse
    glcd_command(0xC0); // Normal COM0~COM63
#endif
    glcd_command(0xA6); // Display normal
    glcd_command(0xA2); // Set bias 1/9 (Duty 1/65)
    glcd_command(0x2F); // Booster, Regulator and Follower on
    glcd_command(0xF8); // Set internal Booster to 4x
    glcd_command(0x00); // Set internal Booster to 4x
    glcd_command(0x27); // Contrast set
    glcd_command(0x81); // Contrast set
    glcd_command(0x10); // Contrast set
    glcd_command(0xAC); // No indicator
    glcd_command(0x00); // No indicator
    glcd_command(0xAF); // Display on
    // End LCD init

    glcd_hw_reset();

    // The screen is the "normal" way up
    glcd_flipped = 0;
}

// Originale Version von edeca
// TODO: Konstanten übernehmen (GLCD_CMD_HORIZONTAL_NORMAL, GLCD_CMD_VERTICAL_REVERSE usw.)

void glcd_init2() {

    // Select the chip
    output_low(GLCD_CS1);

    output_low(GLCD_RESET);

    // Datasheet says "wait for power to stabilise" but gives
    // no specific time!
    delay_ms(50);

    output_high(GLCD_RESET);

    // Datasheet says max 1ms here
    //delay_ms(1);

    // Set LCD bias to 1/9th
    glcd_command(GLCD_CMD_BIAS_9);

    // Horizontal output direction (ADC segment driver selection)
    glcd_command(GLCD_CMD_HORIZONTAL_NORMAL);

    // Vertical output direction (common output mode selection)
    glcd_command(GLCD_CMD_VERTICAL_REVERSE);

    // The screen is the "normal" way up
    glcd_flipped = 0;

    // Set internal resistor.  A suitable middle value is used as
    // the default.
    glcd_command(GLCD_CMD_RESISTOR | 0x3);
    // Power control setting (datasheet step 7)
    // Note: Skipping straight to 0x7 works with my hardware.
    //	glcd_command(GLCD_CMD_POWER_CONTROL | 0x4);
    //	delay_ms(50);
    //	glcd_command(GLCD_CMD_POWER_CONTROL | 0x6);
    //	delay_ms(50);
    glcd_command(GLCD_CMD_POWER_CONTROL | 0x7);
    //	delay_ms(10);
    // Volume set (brightness control).  A middle value is used here
    // also.
    glcd_command(GLCD_CMD_VOLUME_MODE);
    glcd_command(31);
    // Reset start position to the top
    glcd_command(GLCD_CMD_DISPLAY_START);
    // Turn the display on
    glcd_command(GLCD_CMD_DISPLAY_ON);
    // Unselect the chip
    output_high(GLCD_CS1);
}

void glcd_data(int8 data) {

    // A0 is high for display data
    output_high(GLCD_A0);

    // Wird von spi System gesteuert
    // Select the chip
    //output_low(GLCD_CS1);

    spi_write(data);

    // Wird von spi System gesteuert
    // Unselect the chip
    //output_high(GLCD_CS1);

}

void glcd_command(int8 command) {

    // A0 is low for command data
    output_low(GLCD_A0);

    // Wird von spi System gesteuert
    // Select the chip
    //output_low(GLCD_CS1);

    spi_write(command);
    // Wird von spi System gesteuert
    // Unselect the chip
    //output_high(GLCD_CS1);
}

void glcd_flip_screen(int1 flip) {
    if (flip) {
        glcd_command(GLCD_CMD_HORIZONTAL_NORMAL);
        glcd_command(GLCD_CMD_VERTICAL_REVERSE);
        glcd_flipped = 0;
    } else {
        glcd_command(GLCD_CMD_HORIZONTAL_REVERSE);
        glcd_command(GLCD_CMD_VERTICAL_NORMAL);
        glcd_flipped = 1;
    }
}

void glcd_inverse_screen(int1 inverse) {
    if (inverse) {
        glcd_command(GLCD_CMD_DISPLAY_REVERSE);
    } else {
        glcd_command(GLCD_CMD_DISPLAY_NORMAL);
    }
}

void glcd_test_card() {
    int8 p = 0xF0;

    for (int16 n = 1; n <= (SCREEN_WIDTH * SCREEN_HEIGHT / 8); n++) {
        glcd_buffer[n - 1] = p;

        if (n % 4 == 0) {
            char q = p;
            p = p << 4;
            p |= q >> 4;
        }
    }

    glcd_refresh();
}

void glcd_contrast(char resistor_ratio, char contrast) {
    if (resistor_ratio > 7 || contrast > 63) return;

    glcd_command(GLCD_CMD_RESISTOR | resistor_ratio);
    glcd_command(GLCD_CMD_VOLUME_MODE);
    glcd_command(contrast);
}
